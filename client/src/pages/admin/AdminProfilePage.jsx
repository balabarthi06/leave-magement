...
import { Shield, Key, CheckCircle2, Lock, Mail, Sparkles, User as UserIcon, Upload } from 'lucide-react';
...
export const AdminProfilePage = () => {
  ...
  const handlePasswordChange = async (e) => {
    ...
    if (res.success) {
      setSuccessMsg(res.message || 'Admin security password updated successfully!');
      setCurrentPassword('');
      setNewPassword('');
      setConfirmPassword('');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-200">
      <div>
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Admin Settings & Profile</h1>
        <p className="text-xs text-slate-500">Manage administrator privileges and account security</p>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-md p-6 flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="space-y-1 text-center sm:text-left flex-1">
          ...
          <div className="flex items-center justify-center sm:justify-start gap-4 pt-2 text-xs text-slate-400">
            ...
            <span className="flex items-center gap-1">
              <Shield className="w-3.5 h-3.5 text-slate-400" /> Admin ID: {user.employee_id}
            </span>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  value="Admin"
                  disabled
                  className="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs bg-slate-50 text-slate-500 cursor-not-allowed focus:outline-none"
                />
              </div>
